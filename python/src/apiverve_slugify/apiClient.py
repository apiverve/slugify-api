...
class SlugifyAPIClientError(Exception):
...
class ValidationError(SlugifyAPIClientError):
...
class SlugifyAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"text": {"type": "string", "required": True}, "separator": {"type": "string", "required": False, "default": "-"}, "lowercase": {"type": "boolean", "required": False, "default": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the SlugifyAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises SlugifyAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/slugify'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[SlugifyAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns SlugifyData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises SlugifyAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise SlugifyAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise SlugifyAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise SlugifyAPIClientError(f"Request failed: {str(e)}")
